...
class App extends Component {
  // componentWillMount():
  // 主にサーバーへのAPIコールを行うなど、
  // 実際のレンダリングが行われる前にサーバーサイドのロジックを実装するために使用
  ...
  // MetaMask との接続によって得られた除法とコントラクトとの情報を使って描画に使う情報を取得
  async loadBlockchainData() {
    ...
    if (daiTokenData) {
      ...
      window.alert('DaiToken constract not deployed to detected network.')
    }
    ...
    if (dappTokenData) {
      ...
      // dappTokenBalance（ユーザーの Dapp トークンの残高）をString型に変更する
      this.setState({ dappTokenBalance: dappTokenBalance.toString() })
      // ユーザーの Dapp トークンの残高をフロントエンドの Console に出力する
      console.log('dappTokenBalance: ', dappTokenBalance.toString())
    } else {
      window.alert('DappToken contract not deplyoyed to detected network.')
    }
  ...
  async loadWeb3() {
    if (window.ethereum) {
      // ユーザーが Metamask のアカウントを持っていた場合はアドレスを取得
      ...
      // ユーザーが Metamask のアカウントを持っていなかった場合はエラーを返す
  ...
  // TokenFarm.sol のステーキング機能を呼び出す
  stakeTokens = (amount) => {
    if (amount <= 0) {
      window.alert('Please Stake Tokens input over 0')
      return
    }
  ...
  // TokenFarm.sol のアンステーキング機能を呼び出す
